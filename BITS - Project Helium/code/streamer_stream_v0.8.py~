...
###################################	
def launch_vlc(arg):
	subprocess.call("vlc -vvv /windows/d/songs/Echoes.mp3 --sout '#transcode{vcodec=mp2v,vb=800,scale=1,acodec=mpga,ab=128,channels=2}:duplicate{dst=display,dst=rtp{dst=10.42.43.10,mux=ts,port=1234}}' --rc-host localhost:45123",shell=True)
	sleep(2)
...
def initialize():
	...
	#subprocess.Popen("vlc -vvv /home/anshul/01.mp3 --sout '#transcode{vcodec=mp2v,vb=800,scale=1,acodec=mpga,ab=128,channels=2}:duplicate{dst=display,dst=rtp{dst=10.42.43.10,mux=ts,port=1234}}' -I rc --rc-host=\"localhost:45122\"",shell=True)

	subprocess.Popen("vlc -vvv /media/Fun/Movies/01.avi --sout '#transcode{vcodec=mp2v,vb=800,scale=1,acodec=mpga,ab=128,channels=2}:duplicate{dst=display,dst=rtp{dst=172.17.9.47,mux=ts,port=1234}}' -I rc --lua-config=\"rc={host='localhost:49700'}\"",shell=True)
